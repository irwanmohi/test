#!/bin/bash
cleae
echo Installing Websocket-SSH AUTOSCRIPT BY KAIZENVPN
sleep 1
echo Cek Hak Akses...
sleep 0.5
cd
...
wget -O /etc/systemd/system/ws-stunnel.service https://raw.githubusercontent.com/Apeachsan91/server/main/ws-stunnel.service && chmod +x /etc/systemd/system/ws-stunnel.service
...
wget -O /usr/local/bin/ws-dropbear https://raw.githubusercontent.com/Apeachsan91/server/main/ws-dropbear && chmod +x /usr/local/bin/ws-dropbear
...
wget -O /usr/local/bin/ws-stunnel https://raw.githubusercontent.com/Apeachsan91/server/main/ws-stunnel && chmod +x /usr/local/bin/ws-stunnel
